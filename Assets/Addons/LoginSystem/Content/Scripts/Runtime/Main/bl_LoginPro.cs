...
public class bl_LoginPro : bl_LoginProBase
{
    ...
    private void OnAuthenticated(CustomAuthCredentials credentials, LoginUserInfo info)
    {
        ...
        {
            //BannedText.text = string.Format("You has been permanently <b>banned</b> from the game due {0} by {1}.");
        ...
        if (bl_LoginProDataBase.Instance.AutomaticallyLoadScene && info.UserStatus != LoginUserInfo.Status.Banned)//load the scene after login success (without continue menu)
        {
            Continue();
        }
        else
...
#region SignUp
    ...
    IEnumerator RegisterProcess(string user, string nick, string pass, string email)
    {
        ...
        mForm.AddSecureField("coins", 1000);
