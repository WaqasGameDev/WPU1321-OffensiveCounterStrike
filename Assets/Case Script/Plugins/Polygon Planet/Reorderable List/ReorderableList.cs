...
namespace PolygonPlanet.ReorderableList
{
	public class ReorderableList
    {
		...
		public ReorderableList(SerializedProperty list, bool canAdd, bool canRemove, bool draggable) : this(list, canAdd, canRemove, draggable, ElementDisplayType.Auto, null, null)
        {
		}

		public ReorderableList(SerializedProperty list, bool canAdd, bool canRemove, bool draggable, ElementDisplayType elementDisplayType, string elementNameProperty) : 
            this(list, canAdd, canRemove, draggable, elementDisplayType, elementNameProperty, null)
        {
		}
		...
		public void DoList(Rect rect, GUIContent label)
        {
			...
			else
            {
                ...
                {					
					...
					if (list.arraySize > 0)
                    {
						//Update element rects if not dragging.
						if (!dragging)
                        {
		...
		private void DrawElements(Rect rect, Event evt)
        {
			...
			else if (evt.type == EventType.Repaint)
            {
				...
				while (--i > -1)
                {
                    //Fraw dragging elements last as the loop is backwards.
                    DragElement element = dragList[i];					
					if (element.selected)
                    {
