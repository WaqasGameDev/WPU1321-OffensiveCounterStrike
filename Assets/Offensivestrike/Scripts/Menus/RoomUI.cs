...
public class RoomUI : Photon.MonoBehaviour
{
	...
	private void ProcessAutoFire()
	{
		...
		var hitForTactical = hit ? hitInfo.collider.GetComponent<TacticalAI.HitBox>() : null;

		Debug.DrawRay(ray.origin, ray.direction * rayLength, Color.green);


		if ((!hitbox || (hitbox.playerNetwork.playerTeam == rc.ourTeam && !rc.offlineMode && rc.currentGameMode != "FFA")) && !hitForTactical)
        {
			//fireButton.isActive = false;
			GameSettings.mobileFiring = false;
			return;
